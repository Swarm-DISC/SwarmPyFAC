...
#if utils is made into a class, then this should derive from it
class Algorithm: 
    ...
    def split_models(self, baseline, *models):
    ...
    def radial_current(self, delta_B, velocity, delta_time, MU_0=sw.MU_0):
    ...
    def single_sat_fac(self, time, positions, B_res, B_model):
        ...
        irc = self.radial_current(B2-B1, V_VSC, dt)
    ...
    def single_sat_fac_full(self, input_data):
        """ Calculate the Field Aligned Currents on a single satelites path.
        ...
        return single_sat_fac(
            ...
            *self.split_models(input_data['B_base'], *[input_data[key] for key in [
                'B_core', 'B_lithosphere',
                'B_magnetosphere_primary',
                'B_magnetosphere_secondary'] if key in input_data]))


    def fetch_data(self, force_download=False, temp_file='tempdata.cdf',**options):
    ...
    def fac_from_file(self, **options):
        ...
        **options
        ...
        (array([...]), array([[...]]), array([...]), array([...]), array([[...]]), array([[...]]), array([[...]]), array([...]))
        """
        input_data = self.fetch_data(**options)
        input_data['time'] *= 0.001  # conversion to seconds
        return (self.single_sat_fac_full(input_data),
                input_data)


def split_models(baseline, *models):
    return Algorithm().split_models(baseline, *models)

def radial_current(delta_B, velocity, delta_time, MU_0=sw.MU_0):
    return Algorithm().radial_current(delta_B, velocity, delta_time, MU_0=sw.MU_0)

def single_sat_fac(time, positions, B_res, B_model):
    return Algorithm().single_sat_fac(time, positions, B_res, B_model)

def single_sat_fac_full(input_data):
    return Algorithm().single_sat_fac_full(input_data)

def fetch_data(force_download=False, temp_file='tempdata.cdf',**options):
    return Algorithm().fetch_data(force_download=False, temp_file='tempdata.cdf',**options)

def fac_from_file(**options):
    return Algorithm().fac_from_file(**options)
